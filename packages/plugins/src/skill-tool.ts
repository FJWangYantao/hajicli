import type { BaseTool, SkillRegistry, ToolDefinition, ToolExecutionContext } from "@hajicli/core";
...
export class ListSkillResourcesTool implements BaseTool {
  ...
  public async execute(
    ...
    try {
      const result = await this.registry.listResources(
        name,
      ...
      const lines = [
        ...
        ...result.resources.map(
          (resource) => `- ${resource.path} [${resource.kind}, ${resource.size} bytes]`,
        ),
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          path: {
            type: "string",
            description: "Skill 目录内的相对资源路径，例如 references/rules.md",
          },
