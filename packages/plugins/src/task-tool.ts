import type { BaseTool, TaskStatus, TaskStore, ToolDefinition } from "@hajicli/core";
...
export class TaskCreateTool extends StoredTaskTool {
  ...
  readonly definition: ToolDefinition = {
    ...
    function: {
      name: "taskcreate",
      description:
        "逐条创建计划任务。创建前必须先调用 tasklist 检查当前会话是否已有任务，优先复用或修订，不要创建重复任务。第一条提供简短总标题（优先 4-8 个汉字，最多 12 个字符）；最后一条设置 finalize=true 触发用户审批。",
      parameters: {
        type: "object",
        properties: {
          title: {
            type: "string",
            maxLength: 12,
            description: "简短总标题，第一条必填；只写核心目标，不加项目名或“计划”等后缀",
          },
...
export class TaskListTool extends StoredTaskTool {
  ...
  async execute(): Promise<string> {
    return JSON.stringify(
      this.store.getPlan() || { title: "", tasks: [], completedTasks: [] },
      null,
      2,
    );
  }
...
export class TaskFinishTool extends StoredTaskTool {
  ...
  readonly definition: ToolDefinition = {
    ...
    function: {
      name: "taskfinish",
      description:
        "仅能结束 status=in_progress 且已完成实际验证的任务。严格顺序：updatetask(in_progress) → 实施 → 验证 → taskfinish；即使任务实际上已完成，也不得跳过 in_progress。所有任务完成后必须进行一次总验证。",
      parameters: {
        type: "object",
        properties: {
          taskId: { type: "string" },
          verification: {
            type: "string",
            description:
              "已实际执行的验证及结果；调用前确认该任务已通过 updatetask 设为 in_progress",
          },
        },
  ...
  async execute(args: Record<string, unknown>): Promise<string> {
    try {
      const result = this.store.finishTask(
        String(args.taskId || ""),
        String(args.verification || ""),
      );
