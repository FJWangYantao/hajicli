...
export class SubagentTool implements BaseTool {
  ...
  readonly definition: ToolDefinition = {
    ...
    function: {
      name: "subagent",
      description:
        "将一个边界清晰的复杂子任务委派给独立上下文的子代理，只返回最终结论。适合调研、实现或审查；不要用于简单的一步操作。",
      parameters: {
        type: "object",
        properties: {
          ...
          role: {
            type: "string",
            enum: ["research", "implement", "review"],
            description: "子代理角色",
          },
          model: { type: "string", description: "可选：本次子代理使用的模型；不填则继承主 Agent" },
          provider: {
            type: "string",
            enum: ["deepseek", "volcengine"],
            description: "可选：Provider；必须与模型注册信息匹配",
          },
          reasoningEffort: {
            type: "string",
            enum: ["low", "medium", "high", "xhigh", "max"],
            description: "可选：本次子代理的思考强度",
          },
          instructions: {
            type: "string",
            maxLength: MAX_SUBAGENT_INSTRUCTIONS_LENGTH,
            description: "可选：追加到受保护系统提示词后的任务专属要求",
          },
          timeoutMs: {
            type: "integer",
            minimum: 100,
  ...
  execute(args: Record<string, unknown>, context?: ToolExecutionContext): Promise<string> {
    ...
    const reasoningEffort = ["low", "medium", "high", "xhigh", "max"].includes(
      String(args.reasoningEffort || ""),
    )
    ...
    const instructions =
      args.instructions === undefined
        ? undefined
        : normalizeSubagentInstructions(String(args.instructions));
    if (
      typeof args.instructions !== "undefined" &&
      (!instructions || String(args.instructions).trim().length > MAX_SUBAGENT_INSTRUCTIONS_LENGTH)
    ) {
      return Promise.resolve(
        `错误: instructions 长度必须是 1 到 ${MAX_SUBAGENT_INSTRUCTIONS_LENGTH} 个字符。`,
      );
    }
    const timeoutMs = args.timeoutMs === undefined ? undefined : Number(args.timeoutMs);
    if (
      timeoutMs !== undefined &&
      (!Number.isInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 3_600_000)
    ) {
    ...
    if (
      ...
      (!Number.isInteger(maxTokens) ||
        maxTokens < MIN_SUBAGENT_MAX_TOKENS ||
        maxTokens > MAX_SUBAGENT_MAX_TOKENS)
    ) {
      return Promise.resolve(
        `错误: maxTokens 必须是 ${MIN_SUBAGENT_MAX_TOKENS} 到 ${MAX_SUBAGENT_MAX_TOKENS} 之间的整数。`,
      );
    }
    ...
    if (
      ...
      (!Number.isInteger(maxToolCalls) ||
        maxToolCalls < MIN_SUBAGENT_MAX_TOOL_CALLS ||
        maxToolCalls > MAX_SUBAGENT_MAX_TOOL_CALLS)
    ) {
      return Promise.resolve(
        `错误: maxToolCalls 必须是 ${MIN_SUBAGENT_MAX_TOOL_CALLS} 到 ${MAX_SUBAGENT_MAX_TOOL_CALLS} 之间的整数。`,
      );
    }
    return this.handler(
      {
...
export class VerifyAgentTool implements BaseTool {
  ...
  readonly definition: ToolDefinition = {
    ...
    function: {
      name: "verifyagent",
      description:
        "父 Agent 在独立读取、搜索、构建或测试后，验证或拒绝一个已完成的子代理结果。不能直接采信子代理报告。",
      parameters: {
        type: "object",
        properties: {
  ...
  execute(args: Record<string, unknown>): Promise<string> {
    ...
    if (!["verified", "rejected"].includes(verdict))
      return Promise.resolve("错误: verdict 必须是 verified 或 rejected。");
    if (!evidence) return Promise.resolve("错误: verifyagent 必须提供独立验证证据。");
    if (evidenceToolCallIds.length === 0)
      return Promise.resolve("错误: verifyagent 必须提供 evidenceToolCallIds。");
