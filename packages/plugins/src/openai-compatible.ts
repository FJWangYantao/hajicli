  ...
  constructor(config: OpenAICompatibleConfig) {
    if (!config.apiKey) {
      throw new ProviderError(
        `未配置 ${config.providerName || "自定义"} API Key。`,
        config.providerName || "custom",
      );
    }
    if (!config.baseUrl) {
      throw new ProviderError(
        `未配置 ${config.providerName || "自定义"} Base URL。`,
        config.providerName || "custom",
      );
    }
    if (!config.defaultModel) {
      throw new ProviderError(
        `未配置 ${config.providerName || "自定义"} 默认模型。`,
        config.providerName || "custom",
      );
    }
    ...
    return withExponentialBackoff(
      async () => {
        try {
          ...
          const msg = isTimeout
            ? `网络请求超时（${Math.round(getModelTimeoutMs() / 1000)}s），${this.providerName} API 未在规定时间内响应。`
            : error instanceof Error
              ? error.message
              : String(error);
          // 走到这里的一律是传输层瞬时故障（超时/连接失败等），显式标记可重试，
          // 避免本地化消息无法被 isRetryableError 的关键词匹配而漏掉重试。
          throw new ProviderError(msg, this.providerName, undefined, true);
        }
      },
      {
