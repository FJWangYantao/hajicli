...
function parseOptions(args: Record<string, unknown>): ProjectInfoOptions {
  ...
  if (depth !== "summary" && depth !== "detailed")
    throw new Error("depth 只能是 summary 或 detailed。");
  if (args.includeGit !== undefined && typeof args.includeGit !== "boolean")
    throw new Error("includeGit 必须是布尔值。");
  if (args.refresh !== undefined && typeof args.refresh !== "boolean")
    throw new Error("refresh 必须是布尔值。");
...
function isEntrypoint(file: string): boolean {
  ...
  return (
    /(?:^|\/)(?:src\/)?(?:index|main|app|server|cli)\.(?:ts|tsx|js|jsx|mjs|cjs|py|rs|go|java)$/.test(
      normalized,
...
async function readPackageSummary(cwd: string, file: string): Promise<PackageSummary | null> {
  try {
    const parsed = JSON.parse(await fs.readFile(path.join(cwd, file), "utf8")) as Record<
      string,
      unknown
    >;
    const scriptsValue =
    ...
    const workspaces = Array.isArray(workspaceValue)
      ...
      : workspaceValue &&
          ...
          Array.isArray((workspaceValue as Record<string, unknown>).packages)
        ? ((workspaceValue as Record<string, unknown>).packages as unknown[]).filter(
...
async function buildProjectInfo(
  ...
  const packageFiles = configFiles
    .filter((file) => path.posix.basename(file).toLowerCase() === "package.json")
    .slice(0, 30);
  const packages = (
    await Promise.all(packageFiles.map((file) => readPackageSummary(cwd, file)))
  ...
  return {
    ...
    fileScanTruncated,
    languageCounts: [...languageMap.entries()].sort(
      ...
      (error, stdout) => {
        ...
        if (error && !stdout) {
          resolve({
            changedCount: 0,
            changedSample: [],
            unavailable: "不是 Git 仓库或 Git 不可用",
          });
          return;
        }
...
function renderProjectInfo(
  ...
  for (const pkg of info.packages.slice(0, depth === "detailed" ? 20 : 5)) {
    records.push(
      `包 ${pkg.path}: ${pkg.name || "(未命名)"}；scripts=${pkg.scripts.slice(0, detailLimit).join(", ") || "(无)"}${pkg.workspaces.length > 0 ? `；workspaces=${pkg.workspaces.join(", ")}` : ""}`,
    );
  }
  if (info.entrypoints.length > 0)
    records.push(`入口候选=${info.entrypoints.slice(0, detailLimit).join(", ")}`);
  if (info.configFiles.length > 0)
    records.push(
      `配置文件=${info.configFiles.slice(0, depth === "detailed" ? 60 : 20).join(", ")}`,
    );
  if (git) {
    if (git.unavailable) records.push(`Git=${git.unavailable}`);
    else {
      records.push(`Git分支=${git.branch || "(未知)"}；变更数=${git.changedCount}`);
      if (depth === "detailed" && git.changedSample.length > 0)
        records.push(`Git变更样本=${git.changedSample.join(", ")}`);
    }
  }
  if (info.fileScanTruncated)
    records.push(`提示：文件数超过扫描上限 ${MAX_SCANNED_FILES}，语言与目录统计为前缀样本。`);
...
export class ProjectInfoTool implements BaseTool {
  ...
  public readonly definition: ToolDefinition = {
    ...
    function: {
      name: "projectinfo",
      description:
        "快速汇总当前项目的语言、目录、包管理、脚本、配置、入口和 Git 状态，减少进入陌生项目时的重复搜索。",
      parameters: {
        type: "object",
        properties: {
          depth: {
