...
export class McpClient {
  ...
  async connect(): Promise<void> {
    ...
    proc.on("error", (error) =>
      this.failAll(new Error(`无法启动 ${this.config.command}: ${error.message}`)),
    );
...
export class McpTool implements BaseTool {
  ...
  constructor(
    ...
    const desc = (spec.description || `MCP server "${client.serverName}" 提供的工具 ${spec.name}。`)
      .replace(/\s+/g, " ")
      .slice(0, 1024);
    this.definition = {
      type: "function",
...
export class McpManager {
  ...
  private toolSpecs = new Map<
    string,
    Array<{ spec: McpToolSpec; readOnly: boolean; client: McpClient }>
  >();
    ...
    await Promise.all(
      Object.entries(configs).map(async ([name, config]) => {
        ...
        try {
          ...
          this.toolSpecs.set(
            name,
            specs.map((spec) => ({ spec, readOnly: Boolean(config.readOnly), client })),
          );
