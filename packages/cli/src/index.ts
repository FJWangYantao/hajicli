      ...
      if (trimmedInput.startsWith('/')) {
        ...
        if (command === 'rewind') {
          ...
          try {
            ...
            if (!snapshotEngine.rollback(targetMsgNode.snapshotId)) {
              ui.writeLine(colors.boldRed('❌ 代码快照恢复失败，消息历史与工作区均未修改。'));
              ui.writeLine(colors.gray('可能原因：快照缺失，或当前 Git HEAD 已发生变化。'));
              continue;
            }
...
ui.writeLine(colors.red(`未知命令: /${command}。输入 /help 查看帮助。`));
      ...
      const snapshotId = snapshotEngine.createSnapshot(`before user message ${messages.length}`);
