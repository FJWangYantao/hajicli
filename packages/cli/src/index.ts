...
import { queueRewindRefill, REWIND_CONFIRM_DEFAULT } from "./rewind-flow.js";
import {
  applySubagentPreset,
  findSubagentPreset,
  loadSubagentPresets,
...
import {
  ...
  runSubagentWizard,
  type WizardDeps,
} from "./subagent-wizard.js";
import {
  shouldRestartBackgroundInput,
  TerminalInputCancelledError,
  TerminalUI,
...
function detectProviderForModel(
  ...
  if (preferProvider) {
    const declaredByPrefer = isBuiltinProvider(preferProvider)
      ? (preferProvider === "deepseek" ? DEEPSEEK_MODELS : VOLCENGINE_MODELS).some(
...
async function main() {
  ...
  const startupProfileT0 =
    typeof (globalThis as { __hajiPreloadT0?: number }).__hajiPreloadT0 === "number"
  ...
  const markStartupStage = (stage: string): void => {
    ...
    process.stderr.write(
      `[startup] ${stage} ${(performance.now() - startupProfileT0).toFixed(1)}ms\n`,
    );
  };
  markStartupStage("node_boot");
  const cliArgs = process.argv.slice(2);
  handleEarlyCliArgs(cliArgs);

  const providerConfig = loadProviderConfig();
  let volcApiKey =
    process.env.VOLC_API_KEY ||
    process.env.ARK_API_KEY ||
    providerConfig.providers.volcengine?.apiKey;
  let deepseekApiKey = process.env.DEEPSEEK_API_KEY || providerConfig.providers.deepseek?.apiKey;

  const hasCustomProviderKey = Object.entries(providerConfig.providers).some(
    ([name, entry]) => !isBuiltinProvider(name) && Boolean(entry?.apiKey),
  );
  if (!volcApiKey && !deepseekApiKey && !hasCustomProviderKey) {
    console.error(
      colors.boldRed(
        "错误: 请配置 DEEPSEEK_API_KEY / VOLC_API_KEY，或启动后使用 /provider add 添加自定义提供商。",
      ),
    );
    process.exit(1);
  }
  ...
  const buildProvider = (
    ...
    const providerName =
      providerOverride || detectProviderForModel(modelValue, providerConfig, preferProvider);
    if (providerName === "volcengine") {
      ...
      return new ObservableModelProvider(
        new VolcengineProvider({
          apiKey: volcApiKey,
          baseUrl: resolveProviderSetting(
            "volcengine",
            "baseUrl",
            process.env.VOLC_BASE_URL || process.env.ARK_BASE_URL,
  ...
  const envModel = (
    deepseekApiKey
      ...
      : resolveProviderSetting(
          "volcengine",
          "model",
          process.env.VOLC_MODEL || process.env.ARK_MODEL,
          undefined,
          providerConfig,
        )
  )
  ...
  const modelInBuiltinRegistry =
    DEEPSEEK_MODELS.some((m) => m.value === selectedModel) ||
  ...
  const modelDeclaredByCustomProvider = Object.entries(providerConfig.providers).some(
    ([name, entry]) => !isBuiltinProvider(name) && entry?.models?.includes(selectedModel),
  );
  const modelOwnedByBuiltinOnly = modelInBuiltinRegistry && !modelDeclaredByCustomProvider;
  let currentProviderName: string =
    savedProvider &&
    providerHasApiKey(savedProvider) &&
  ...
  let reasoningEffort: ReasoningEffort = isReasoningEffort(configuredEffort)
    ? configuredEffort
    : isReasoningEffort(savedPreference?.reasoningEffort)
      ? savedPreference!.reasoningEffort
      : "medium";
  ...
  let permissionMode: PermissionMode = isPermissionMode(savedPreference?.permissionMode)
    ...
    : "default";
  let riskThreshold: RiskLevel = (
    ["low", "medium", "high"].includes(savedPreference?.riskThreshold || "")
  ...
  hookEngine.register("PreToolUse", async (ctx) => {
    ...
    if (checkResult.action === "prompt") {
      ...
      if (!approved) {
        ui.writeLine(
          `  ${colors.boldRed("✕")} ${colors.purple(ctx.toolName!)}${displayArgs} ${colors.gray("(已拒绝执行)")}`,
        );
    ...
    if (checkResult.action === "deny") {
      ...
      ui.writeLine(
        `  ${colors.boldRed("🛡️ [Auto安全拦截]")} ${colors.purple(ctx.toolName!)}${displayArgs} ${colors.red(`(评级: ${checkResult.riskLevel} - ${autoDeniedReason})`)}`,
      );
  ...
  hookEngine.register("PostToolUse", async (ctx) => {
    const isApproved =
      !ctx.toolOutput?.startsWith("错误:") && !ctx.toolOutput?.startsWith("[安全引擎拒绝拦截]");
  ...
  hookEngine.register("UserPromptSubmit", async (ctx) => {
    ...
    if (shouldTriggerAutoCompaction(usedTokens, thresholds, autoCompactionArmed)) {
      ...
      ui.writeLine(
        colors.gray(
          `🧹 上下文约 ${usedTokens.toLocaleString()} / ${thresholds.contextWindowTokens.toLocaleString()} tokens，开始自动压缩...`,
        ),
      );
      ...
      const layers =
        result.layersApplied.length > 0 ? result.layersApplied.join(" -> ") : "无需变更";
      ui.writeLine(
        colors.gray(
          `✓ 自动压缩完成（${layers}），tokens 约 ${result.originalTokens.toLocaleString()} ➔ ${result.compactedTokens.toLocaleString()}。`,
        ),
      );
  ...
  async function summarizeMessagesForCompaction(sourceMessages: ChatMessage[]): Promise<string> {
    const transcript = sourceMessages
      .map((message, index) => JSON.stringify({ index, ...message }))
      .join("\n");
  ...
  let runSubagent: (request: SubagentRequest, context?: ToolExecutionContext) => Promise<string> =
    async () => "错误: 子代理运行时尚未初始化。";
  ...
  if (Object.keys(mcpConfigs).length > 0) {
    ...
    for (const status of mcpManager.getServerStatuses()) {
      if (status.state === "failed") {
        mcpStartupWarnings.push(
          `MCP server "${status.name}" 启动失败: ${status.error ?? "未知错误"}`,
        );
  ...
  const activeTools = () =>
    permissionMode === "plan"
      ? tools.filter(
          (tool) =>
            permissionEngine.isReadOnlyTool(tool.name) ||
            ["subagent", "verifyagent"].includes(tool.name) ||
            ["taskcreate", "tasklist", "updatetask"].includes(tool.name),
        )
      : tools;
  ...
  const createSystemPrompt = () =>
    systemPromptManager.generatePrompt({
      cwd: process.cwd(),
  ...
  const syncTaskPlanUI = (recentlyCompleted?: { id: string; content: string }): void => {
    ...
    ui.setTaskPlan({
      title: plan.title,
      tasks: plan.tasks,
      completedTasks: recentlyCompleted ? [{ ...recentlyCompleted, status: "completed" }] : [],
    });
  };
  syncTaskPlanUI();
  ...
  const syncAgentPanel = (agents: AgentRecord[]): void => {
    ui.setAgentPanel(
      agents
        .filter((agent) =>
          ["queued", "running", "awaiting_verification", "failed", "aborted"].includes(
            agent.status,
          ),
        )
        .map((agent) => ({
  ...
  const agentManager = new AgentManager({
    ...
    onNotification: (notification) => {
      ...
      const mark =
        notification.type === "completed" ? colors.boldGreen("✓") : colors.boldYellow("!");
      ui.writeLine(
        `${mark} ${colors.gray(`[${notification.agentId}] ${sanitizeTerminalText(notification.message)}`)}`,
      );
    },
  });
  ...
  const refreshAgentVerificationContext = (targetMessages: ChatMessage[]): boolean => {
    ...
    while (true) {
      ...
      const after =
        end < 0 ? "" : content.slice(end + AGENT_VERIFICATION_CONTEXT_END.length).trimStart();
  ...
  const toolExecutor = new SharedToolExecutor({
    ...
    onToolProgress: ({ toolName, progress, context }) => {
      ...
      const batchPrefix =
        context.batchSize && context.batchSize > 1
  ...
  const subagentRunner = new SubagentRunner({
    ...
    getTools: (context) =>
      context.agentAccess === "readonly" || context.permissionMode === "plan"
        ? tools.filter((tool) => permissionEngine.isReadOnlyTool(tool.name))
        : tools.filter(
            (tool) =>
              !["subagent", "verifyagent"].includes(tool.name) &&
              !tool.name.toLowerCase().startsWith("task"),
          ),
    executeTool: async (toolCall, args, context) => {
      ...
      ui.writeLine(
        `  ${resultMark} ${colors.gray(`[${context.agentId}]`)} ${colors.purple(toolCall.function.name)} ${colors.gray(`(${result.duration}ms)`)}`,
      );
  ...
  const resolveSubagentRequest = (request: SubagentRequest): SubagentRequest => {
    ...
    if (
      request.provider &&
      !isBuiltinProvider(request.provider) &&
      !providerConfig.providers[request.provider]?.apiKey
    ) {
    ...
    if (descriptor && request.provider && request.provider !== descriptor.provider) {
      throw new Error(
        `Provider ${request.provider} 与模型 ${model} 不匹配，应使用 ${descriptor.provider}`,
      );
    }
    const provider =
      request.provider ||
      descriptor?.provider ||
      (!explicitModel ? currentProviderName : undefined);
    ...
    if (
      ...
      request.instructions.trim().length > MAX_SUBAGENT_INSTRUCTIONS_LENGTH
    ) {
  ...
  runSubagent = (request, context) => {
    ...
    if (
      ...
      !taskStore.getPlan()?.tasks.some((task) => task.id === resolvedRequest.taskId)
    ) {
    ...
    const launch = agentManager.launch(
      ...
      ({ agentId, signal, maxTokens, maxToolCalls }) =>
        subagentRunner.runResult(
          {
            ...resolvedRequest,
            agentId,
            maxTokens,
  ...
  verifyAgent = async (input) => {
    try {
      const agent = agentManager.verify(
        input.agentId,
        input.verdict,
        input.evidence,
  ...
  const launchManualAgent = (request: SubagentRequest, background: boolean) => {
    ...
    if (background && resolvedRequest.role === "implement") {
      throw new Error(
        "第一版后台 Agent 仅支持只读 research/review，写入型后台 Agent 将在 Worktree 版本实现",
      );
    }
    const access =
      background || resolvedRequest.role !== "implement" || permissionMode === "plan"
        ? "readonly"
        : "workspace-write";
  ...
  const permissionCycleList: PermissionMode[] = [
    "plan",
    "default",
    "accept-edit",
    "auto",
  ...
  ui.onShiftTab(() => {
    ...
    void applyPermissionMode(permissionCycleList[nextIdx]).catch((error) => {
      ui.writeLine(
        colors.red(`切换权限模式失败: ${error instanceof Error ? error.message : String(error)}`),
      );
    });
  });
  ...
  const injectAgentNotifications = (): void => {
    const notifications = agentManager
      .drainNotifications()
      .filter((notification) => notification.background);
  ...
  try {
    ...
    while (true) {
      ...
      if (trimmedInput.startsWith("/")) {
        ...
        if (command === "exit" || command === "quit") {
          ...
          if (abortedAgents > 0)
            ui.writeLine(colors.gray(`已中止 ${abortedAgents} 个后台 Agent。`));
        ...
        if (command === "mcp") {
          ...
          if (statuses.length === 0) {
            ui.writeLine(
              colors.gray(
                "未配置 MCP server。在 .haji/config.json（用户级 ~/.haji/ 或项目级）添加 mcpServers 字段：",
              ),
            );
            ui.writeChat(
              colors.gray(
            ...
            ui.writeLine(
              colors.gray(
                "可选字段：env（环境变量）、readOnly: true（工具视为只读，免审批）、enabled: false（禁用）、callTimeoutMs。",
              ),
            );
          } else {
            const stateLabel = (s: (typeof statuses)[number]) =>
              s.state === "running"
            ...
            for (const status of statuses) {
              const ro =
                status.state === "running" && mcpConfigs[status.name]?.readOnly
                  ? colors.gray(" · 只读")
                  : "";
              lines.push(
                `  ${colors.purple(status.name.padEnd(20))} ${stateLabel(status)}${ro} · ${status.toolCount} 个工具`,
              );
              if (status.state === "failed" && status.error)
                lines.push(colors.red(`    ${status.error.replace(/\s+/g, " ").slice(0, 160)}`));
        ...
        if (command === "skills") {
          ...
          const loaded = new Map(
            skillRegistry.getLoaded("main").map((skill) => [skill.name, skill]),
          );
          const skills = skillRegistry.list();
          if (skills.length === 0) {
            ui.writeLine(
              colors.gray(
                "暂无 Skill。项目目录：.haji/skills；用户目录：%USERPROFILE%/.haji/skills。",
              ),
            );
          } else {
            ui.writeChat(
        ...
        if (command === "skill") {
          ...
          const toolCall: ToolCall = {
            ...
            function: {
              name: "loadskill",
              arguments: JSON.stringify({ name, args: skillArgs || undefined }),
            },
          };
          const result = await toolExecutor.execute(
            "loadskill",
            { name, args: skillArgs || undefined },
            {
              toolCallId: toolCall.id,
              permissionMode,
              riskThreshold,
          ...
          try {
            ...
            if (!parsed.description) {
              ...
              if (parsed.preset) {
                ...
                if (!found) {
                  ui.writeLine(
                    colors.red(`预设不存在: ${parsed.preset}。使用 /preset 查看或添加。`),
                  );
                  continue;
                }
                initialPresetName = found.name;
              }
              const wizardState = await runSubagentWizard(presets, initialPresetName, wizardDeps);
              parsed = {
              ...
              const saveName = (
                await ui.readInput({
                  prompt: `${colors.gray("保存为预设 (输入名称或回车跳过)")} › `,
                })
              ).trim();
              if (saveName) {
                if (persistPreset(buildPresetFromParsed(saveName, parsed))) {
                  ui.writeLine(
                    colors.green(
                      `✓ 已保存预设：${saveName}（下次可用 /subagent preset:${saveName} 复用）`,
                    ),
                  );
                } else {
              ...
              ui.writeLine(
                colors.gray(`已应用预设：${preset.name}（${formatPresetSummary(preset)}）`),
              );
            }
            if (!parsed.description) {
            ...
            ui.writeLine(
              colors.cyan(
                `🤖 ${launch.agent.id} ${parsed.background ? "已在后台排队/启动" : "已在前台启动"}。`,
              ),
            );
            if (parsed.background) continue;
            ui.onEsc(() => {
              agentManager.abort(launch.agent.id);
            });
            ...
            if (error instanceof TerminalInputCancelledError) {
              ...
              ui.writeLine(
                colors.red(
                  `启动子代理失败: ${error instanceof Error ? error.message : String(error)}`,
                ),
              );
            }
          }
        ...
        if (command === "preset" || command === "presets") {
          ...
          if (!action || action === "list") {
            if (presets.length === 0) {
              ui.writeLine(
                colors.gray(
                  "暂无 subagent 预设。使用 /preset add 添加，或在 /subagent 交互式流程中保存。",
                ),
              );
            } else {
              ...
              for (const preset of presets) {
                ui.writeLine(
                  `  ${colors.purple(preset.name)} ${colors.gray(formatPresetSummary(preset))}`,
                );
              }
            }
            continue;
          ...
          if (action === "edit" || action === "update") {
            ...
            if (!rawArgs) {
              ui.writeLine(
                colors.red(
                  "用法: /preset edit <name> [--effort ...] [--max-tokens ...] ...（不带选项时交互式修改）",
                ),
              );
              continue;
            }
            try {
              ...
              const patch: Partial<Omit<SubagentPreset, "name">> = {
                ...
                ...(parsedPreset.reasoningEffort
                  ? { reasoningEffort: parsedPreset.reasoningEffort }
                  : {}),
                ...(parsedPreset.instructions !== undefined
                  ? { instructions: parsedPreset.instructions }
                  : {}),
                ...(parsedPreset.timeoutMs !== undefined
                  ? { timeoutMs: parsedPreset.timeoutMs }
                  : {}),
                ...(parsedPreset.maxTokens !== undefined
                  ? { maxTokens: parsedPreset.maxTokens }
                  : {}),
                ...(parsedPreset.maxToolCalls !== undefined
                  ? { maxToolCalls: parsedPreset.maxToolCalls }
                  : {}),
              };
              ...
              if (Object.keys(patch).length === 0) {
                ...
                if (wizardState.instructions !== undefined)
                  patch.instructions = wizardState.instructions;
              ...
              if (saveSubagentPresets(updateSubagentPreset(presets, target.name, patch))) {
                ui.writeLine(
                  colors.green(
                    `✓ 已更新预设：${target.name}（/preset show ${target.name} 查看详情）`,
                  ),
                );
              } else {
                ui.writeLine(colors.yellow("⚠️ 预设保存失败（请检查磁盘权限）。"));
              ...
              if (error instanceof TerminalInputCancelledError) {
                ...
                ui.writeLine(
                  colors.red(
                    `编辑预设失败: ${error instanceof Error ? error.message : String(error)}`,
                  ),
                );
              }
            }
          ...
          if (action === "remove" || action === "rm" || action === "delete") {
            ...
            const next = presets.filter(
              (item) => item.name.toLowerCase() !== existing.name.toLowerCase(),
            );
          ...
          if (action === "add") {
            ...
            try {
              ...
              if (rawArgs) {
                ...
                const wizardState = await runPresetWizard({}, wizardDeps);
              ...
              if (persistPreset(presetToAdd)) {
                ui.writeLine(
                  colors.green(
                    `✓ 已保存预设：${presetToAdd.name}（/preset show ${presetToAdd.name} 查看详情）`,
                  ),
                );
              } else {
                ui.writeLine(colors.yellow("⚠️ 预设保存失败（请检查磁盘权限）。"));
              ...
              if (error instanceof TerminalInputCancelledError) {
                ...
                ui.writeLine(
                  colors.red(
                    `添加预设失败: ${error instanceof Error ? error.message : String(error)}`,
                  ),
                );
              }
            }
        ...
        if (command === "agents") {
          ...
          if (action === "clear") {
            ui.writeLine(
              colors.gray(`已清理 ${agentManager.clearFinished()} 条已结束 Agent 记录。`),
            );
            continue;
          }
          ...
          try {
            ...
            if (selection.secondaryValue === "stop") {
              ui.writeLine(
                agentManager.abort(selection.value)
        ...
        if (command === "resume") {
          ...
          try {
            ...
            if (loaded && loaded.messages.length > 0) {
              ...
              appendHistoryLine(
                colors.boldGreen(
                  `✓ 已恢复会话：「${loaded.title}」 (${loaded.messages.length} 条上下文)`,
                ),
              );
              ...
              for (const m of messages) {
                ...
                if (m.role === "user" && typeof m.content === "string") {
                  ...
                  const isError =
                    ...
                    (m.content.startsWith("错误:") || m.content.startsWith("执行出错:"));
                  if (isError) {
        ...
        if (command === "rewind") {
          ...
          try {
            ...
            const targetContent =
              typeof targetMsgNode?.content === "string" ? targetMsgNode.content : "";

            if (!targetMsgNode?.snapshotId) {
              ui.writeLine(colors.yellow("⚠️ 该历史节点没有安全代码快照，未执行任何回退。"));
              ui.writeLine(
                colors.gray("旧会话节点无法精确恢复工作区；请从升级后的新消息开始使用 /rewind。"),
              );
              continue;
            }
            ...
            if (!rollbackResult.ok) {
              ...
              ui.writeLine(
                colors.gray(rollbackResult.reason || "快照缺失，或当前 Git HEAD 已发生变化。"),
              );
              continue;
            }
            ...
            const rollbackSummary =
              rollbackResult.revertedPaths.length > 0
                ? `，并撤销了本会话修改的 ${rollbackResult.revertedPaths.length} 个文件`
                : "；未改动工作区文件";
            ui.writeLine(
              colors.boldGreen(`↺ 已成功退回历史至节点 #${targetMsgIndex}${rollbackSummary}。`),
            );
            if (rollbackResult.preservedPaths.length > 0) {
              ui.writeLine(
                colors.yellow(
                  `⚠️ 已保留 ${rollbackResult.preservedPaths.length} 个后来被外部修改的文件，避免覆盖你的改动。`,
                ),
              );
        ...
        if (command === "compact") {
          ...
          updateStatusUI();
          const layersStr =
            result.layersApplied.length > 0 ? result.layersApplied.join(" -> ") : "已处于精简状态";
          ui.writeLine(colors.boldGreen(`✓ 上下文压缩完成！(${layersStr})`));
          ui.writeLine(
            colors.cyan(
              `  字符占用: ${result.originalChars.toLocaleString()} ➔ ${result.compactedChars.toLocaleString()} (释放了 ${result.freedPercentage}% 空间)`,
            ),
          );
          ui.writeLine(
            colors.cyan(
              `  Token 估算: ${result.originalTokens.toLocaleString()} ➔ ${result.compactedTokens.toLocaleString()}`,
            ),
          );
          if (result.summaryMode === "fallback") {
            ui.writeLine(
              colors.yellow("⚠️ 模型摘要调用失败，当前结果为本地降级摘要；完整记录仍已落盘。"),
            );
          }
          continue;
        }
        ...
        if (command === "model") {
          try {
            ...
            const newProviderName = detectProviderForModel(
              newModel,
              providerConfig,
              currentProviderName,
            );
            ...
            ui.writeLine(
              colors.green(
                `模型：${selectedModel} · 思考强度：${reasoningEffort} · 提供商：${providerLabel(currentProviderName)}`,
              ),
            );
          } catch (error) {
            if (error instanceof TerminalInputCancelledError) {
        ...
        if (command === "provider" || command === "providers") {
          ...
          const hasUserScope = parts.some(
            (part) => part.toLowerCase() === "--global" || part.toLowerCase() === "--user",
          );
          ...
          if (action && action !== "add" && action !== "set" && action !== "unset") {
            ...
            if (!targetKey) {
              ui.writeLine(
                colors.red(
                  `未配置 ${providerLabel(targetName)} API Key。使用 /provider add 引导配置。`,
                ),
              );
              continue;
            }
            const targetModel =
              availableModels.find((m) => m.value === selectedModel && m.provider === targetName)
                ?.value ||
              (targetName === "deepseek"
                ? DEEPSEEK_MODELS[0].value
                : targetName === "volcengine"
                  ? VOLCENGINE_MODELS[0].value
                  : providerConfig.providers[targetName]?.model ||
                    providerConfig.providers[targetName]?.models?.[0] ||
                    "");
            if (!targetModel) {
              ui.writeLine(
                colors.red(
                  `${providerLabel(targetName)} 未配置任何模型。使用 /provider add 补充模型列表。`,
                ),
              );
              continue;
            }
            ...
            updateStatusUI();
            ui.writeLine(
              colors.green(`已切换到 ${providerLabel(targetName)}，模型：${selectedModel}。`),
            );
            continue;
          }
          ...
          if (action === "add") {
            try {
              while (true) {
                const nameInput = (
                  await ui.readInput({ prompt: "Provider 名称（如 openai、moonshot；留空取消）" })
                ).trim();
                if (!nameInput) {
                ...
                if (providerConfig.providers[lowerName]?.apiKey) {
                  ui.writeLine(
                    colors.yellow(`已存在 ${providerLabel(lowerName)} 的配置，继续输入将覆盖。`),
                  );
                }
                const baseUrl = (
                  await ui.readInput({
                    prompt: "Base URL（OpenAI 兼容端点，如 https://api.openai.com/v1；留空取消）",
                  })
                ).trim();
                if (!baseUrl) {
                  ui.writeLine(colors.gray("已取消添加 provider。"));
                ...
                if (normalizedUrl.note) {
                  ui.writeLine(
                    colors.yellow(`提示：${normalizedUrl.note}，已自动修正为 ${normalizedUrl.url}`),
                  );
                }
                ...
                const modelListInput = (
                  await ui.readInput({
                    prompt: "模型名称（多个用分号分隔，如 gpt-4o;gpt-4o-mini；留空取消）",
                  })
                ).trim();
                ...
                if (!result.ok) {
                  ui.writeLine(
                    colors.red(
                      `✗ 连通性测试失败：${result.error}${result.ms !== undefined ? `（${result.ms}ms）` : ""}`,
                    ),
                  );
                  if (result.normalizedNote) {
                    ui.writeLine(
                      colors.yellow(`提示：${result.normalizedNote}（实际请求 ${result.usedUrl}）`),
                    );
                  }
                  const retry = (await ui.readInput({ prompt: "输入 r 重新配置，输入 c 取消" }))
                    .trim()
                    .toLowerCase();
                  if (retry === "c" || retry === "") {
                    ui.writeLine(colors.gray("已取消添加 provider。"));
                ...
                ui.writeLine(
                  colors.green(
                    `✓ 已添加 ${providerLabel(lowerName)}（${configScopeLabel}，连通性测试通过，${result.ms}ms）。`,
                  ),
                );
                ui.writeLine(colors.gray(`配置文件：${providerConfigPath(configScope)}`));
                ui.writeLine(
                  colors.gray(
                    `模型：${models.join("、")}。输入 /provider ${lowerName} 切换到该提供商。`,
                  ),
                );
                ui.writeLine(
                  colors.gray(
                    "注意：API Key 以明文存储于本地 .haji/config.json（已被 git 忽略），请勿共享该文件。",
                  ),
                );
                break;
              }
          ...
          if (action === "set") {
            ...
            try {
              const inlineArguments = parts
                .slice(3)
                .filter(
                  (part) => !["--project", "--global", "--user"].includes(part.toLowerCase()),
                );
              ...
              const baseUrlInput = (
                await ui.readInput({
                  prompt: `Base URL（当前: ${current.baseUrl || "未设置"}，留空保留）`,
                })
              ).trim();
              ...
              const modelInput = (
                await ui.readInput({
                  prompt: `默认模型（当前: ${current.model || "未设置"}，留空保留）`,
                })
              ).trim();
              const modelsInput = (
                await ui.readInput({
                  prompt: `模型列表（当前: ${current.models?.join("、") || "未设置"}，多个用分号分隔，留空保留）`,
                })
              ).trim();
              ...
              ui.writeLine(
                colors.green(
                  `✓ 已保存 ${label} 的${configScopeLabel}配置（${providerConfigPath(configScope)}）。`,
                ),
              );
              if (
                configScope === "user" &&
                Object.keys(loadProviderConfigScope("project").providers[name] || {}).length > 0
              ) {
                ui.writeLine(
                  colors.yellow("当前项目仍有同名项目级配置，会继续覆盖对应的全局字段。"),
                );
              }
              ui.writeLine(
                colors.gray(
          ...
          if (action === "unset") {
            ...
            if (name === "deepseek") {
              deepseekApiKey =
                process.env.DEEPSEEK_API_KEY || providerConfig.providers.deepseek?.apiKey;
            } else if (name === "volcengine") {
              volcApiKey =
                process.env.VOLC_API_KEY ||
                process.env.ARK_API_KEY ||
            ...
            if (currentProviderName === name && !keyAfter) {
              ui.writeLine(
                colors.yellow(
                  `当前正在使用 ${label}，但已无可用 API Key。建议 /provider add 重新配置或切换提供商。`,
                ),
              );
            }
            continue;
          ...
          const sourceFor = (
            name: string,
            fields: Array<keyof ProviderConfig["providers"][string]>,
          ): string => {
            if (fields.some((field) => projectConfig.providers[name]?.[field] !== undefined))
              return "项目级";
            if (fields.some((field) => userConfig.providers[name]?.[field] !== undefined))
              return "全局";
            return "默认";
          };
          const allNames = [
            ...PROVIDER_NAMES,
            ...Object.keys(providerConfig.providers).filter((n) => !PROVIDER_NAMES.includes(n)),
          ];
          for (const name of allNames) {
            ...
            rows.push(
              `    API Key: ${key ? colors.green(masked) + colors.gray(` [${source}]`) : colors.red("未配置")}`,
            );
            rows.push(
              `    Base URL: ${entry.baseUrl || "默认"} ${colors.gray(`[${sourceFor(name, ["baseUrl"])}]`)}`,
            );
            const modelLabel = entry.models?.length
              ? entry.models.join("、")
              : entry.model || "默认";
            rows.push(
              `    模型: ${modelLabel} ${colors.gray(`[${sourceFor(name, ["models", "model"])}]`)}`,
            );
          }
          rows.push(
            colors.gray(
              `当前模型：${selectedModel} · 提供商：${providerLabel(currentProviderName)}`,
            ),
          );
          rows.push(
            colors.gray(
              "/provider add 引导添加；set/unset 默认全局，附加 --project 操作当前项目；/provider <name> 切换。",
            ),
          );
        ...
        if (command === "permission" || command === "perm") {
          ...
          if (!reqMode) {
            try {
              ...
              if (
                selection.secondaryValue &&
                ["low", "medium", "high"].includes(selection.secondaryValue)
              ) {
          ...
          ui.writeLine(
            colors.green(
              `🛡️  系统权限已设置为 [${permissionMode}] (Auto 危险阈值: ${riskThreshold})`,
            ),
          );
        ...
        if (command === "help") {
          ...
          const helpLines = [
            ...
            ...tools.map(
              (t) =>
                `  ⚙️  ${colors.blue(t.name.padEnd(20))} : ${t.definition.function.description}`,
            ),
            "",
            "",
          ];
        ...
        messages.push(
          {
            ...
            content: "",
            reasoning_content: "",
            tool_calls: [manualSkillExchange.toolCall],
          },
          {
            role: "tool",
            content: manualSkillExchange.output,
            tool_call_id: manualSkillExchange.toolCall.id,
          },
        );
      }
      updateStatusUI();
      ...
      const commitPromptForRequest = () => {
        ...
        const snapshotId = snapshotEngine.createAnchor(
          `before user message ${messagesBeforePrompt.length}`,
        );
        ...
        if (isFirstUserMsg) {
          sessionManager
            .generateTitleAsync(promptInput, async (prompt) => {
              let fullTitleText = "";
      ...
      while (keepCalling) {
        ...
        if (invalidToolCall) {
          ...
          ui.writeLine(
            colors.boldYellow(
              `⚠️ 已拦截不完整的 ${toolName} 工具调用，未执行、未写入工具调用历史。`,
            ),
          );
          ...
          if (malformedToolRecoveryAttempts <= 1) {
            ...
            ui.writeLine(
              colors.red(
                "连续两次收到不完整工具参数，已停止自动重试，避免无限循环。请缩短单次写入内容后重试。",
              ),
            );
            keepCalling = false;
          }
        ...
        if (finishReason === "length") {
          ...
          ui.writeLine(
            colors.yellow(
              `⚠️ 上游 Provider 以 length 结束${usedText}，本轮内容可能不完整；Haji 未设置单次输出上限。`,
            ),
          );
        }
        ...
        if (toolCalls && toolCalls.length > 0) {
          ...
          const anchorSnapshotId = [...messages]
            .reverse()
          ...
          for (const batch of batches) {
            ...
            for (const result of batchRun.results) {
              ...
              if (execution.blocked || toolOutput.startsWith("执行出错:")) {
                ui.writeLine(
                  `  ${colors.boldRed("❌")} ${colors.purple(toolName)}${displayArgs} ${colors.red(`(${safeToolOutput})`)}`,
                );
              } else {
                ui.writeLine(
                  `  ${colors.boldGreen("✓")} ${colors.purple(toolName)}${displayArgs} ${colors.gray(`(${execution.duration}ms)`)}`,
                );
              }
          ...
          if (isTurnAborted) {
            ...
            if (!planReviewSummaryRequested) {
              messages.push({
                role: "user",
                content:
                  "[系统计划审阅要求] Todo 已创建完成。现在请停止调用工具，单独输出一份供用户审阅的简洁方案正文，说明总体思路、关键改动、风险和验证方式。不要只复述 Todo，不要执行计划。",
              });
          ...
          if (permissionMode === "plan" && planReadyForReview) {
              ...
              try {
                  ...
                  items: [
                    {
                      value: "auto",
                      label: "Auto Execute",
                      description: "自动执行安全操作，高风险操作由安全引擎拦截",
                    },
                    {
                      value: "manual",
                      label: "Approve Manually",
                      description: "每个修改型工具都先请求你的批准",
                    },
                    {
                      value: "no",
                      label: "No, Revise Plan",
                      description: "留在 Plan Mode，继续修改计划",
                    },
                  ],
                  selectedValue: "auto",
                });
                ...
                if (review.value === "auto" || review.value === "manual") {
                  ...
                  messages.push({
                    role: "user",
                    content:
                      "[系统工作流通知] 用户已批准当前计划。必须先调用 tasklist 读取当前真实步骤；每个任务都严格按 updatetask(in_progress) → 实施 → 实际验证 → taskfinish 执行，即使任务看起来已完成也不得跳过 in_progress。每完成一项重新检查剩余任务是否需要更新；全部完成后执行一次总验证，通过后总结改动。",
                  });
                  sessionManager.saveCurrentSession(messages);
                  updateStatusUI();
                  ui.writeLine(
                    colors.green(`✓ 计划已批准，已切换到 [${permissionMode}] 并继续执行。`),
                  );
                  keepCalling = true;
                } else {
                  ui.writeLine(
                    colors.gray("计划未批准，仍处于 Plan Mode；你可以继续要求调整计划。"),
                  );
                  keepCalling = false;
                }
              } catch (error) {
            ...
            const unverifiedAgents = agentManager
              .list()
              .filter((agent) => agent.status === "awaiting_verification" && agent.result);
            if (unverifiedAgents.length > 0) {
              const ids = unverifiedAgents.map((agent) => agent.id).join(", ");
              ui.writeLine(
                colors.yellow(
                  `⚠️ 子代理结果尚未独立验证：${ids}。主 Agent 将继续验证，当前结论不能视为完成。`,
                ),
              );
              refreshAgentVerificationContext(messages);
              sessionManager.saveCurrentSession(messages);
    ...
    try {
      ...
      if (
        distillSummary.statisticalInstincts > 0 ||
        distillSummary.llmInstincts > 0 ||
        distillSummary.memoryCandidates > 0
      ) {
        const parts = [
          `统计 ${distillSummary.statisticalInstincts}`,
          `LLM ${distillSummary.llmInstincts}`,
          `强化 ${distillSummary.reinforced}`,
        ];
        if (distillSummary.memoryCandidates > 0)
          parts.push(`记忆候选 ${distillSummary.memoryCandidates}`);
        console.log(
          colors.gray(
            `🧠 经验提炼：${parts.join(" / ")}${distillSummary.llmTriggered ? "" : "（LLM 未触发）"}`,
          ),
        );
        if (distillSummary.memoryCandidates > 0) {
