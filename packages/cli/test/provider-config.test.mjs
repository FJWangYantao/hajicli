...
test("save defaults to user config and merges fields", () => {
  withIsolatedPaths(() => {
    assert.equal(
      saveProviderConfig("deepseek", { apiKey: "sk-a", baseUrl: "https://gw.example.com/v1" }),
      true,
    );
...
test("explicit project config overrides user config field by field", () => {
  withIsolatedPaths(({ home }) => {
    ...
    assert.equal(
      saveProviderConfig(
        "deepseek",
        { apiKey: "sk-project", baseUrl: "https://inner.example.com/v1" },
        "project",
      ),
      true,
    );
...
test("resolve precedence is env > config > fallback", () => {
  withIsolatedPaths(() => {
    ...
    assert.equal(
      resolveProviderSetting("deepseek", "baseUrl", undefined, "https://fallback.example.com"),
    ...
    const config = loadProviderConfig();
    assert.equal(
      resolveProviderSetting("deepseek", "apiKey", undefined, undefined, config),
      "sk-config",
    );
  });
});
...
test("empty string fields are cleared and fall back to user config", () => {
  withIsolatedPaths(({ home }) => {
    ...
    assert.equal(
      config.providers.deepseek.baseUrl,
      "https://inner.example.com/v1",
      "未清除的字段保留",
    );
  });
});
...
test("custom provider names are saved, loaded and unset", () => {
  withIsolatedPaths(() => {
    ...
    assert.deepEqual(
      updated.providers.openai.models,
      ["gpt-4o", "gpt-4o-mini"],
      "models 在部分更新时保留",
    );
...
test("normalizeBaseUrl strips trailing slashes and endpoint suffixes", () => {
  // 尾部斜杠
  assert.deepEqual(normalizeBaseUrl("https://api.openai.com/v1/"), {
    url: "https://api.openai.com/v1",
  });
  // 完整端点
  const full = normalizeBaseUrl("https://api.openai.com/v1/chat/completions");
  assert.equal(full.url, "https://api.openai.com/v1");
  assert.ok(full.note?.includes("chat/completions"));
  // 重复端点（用户可能粘贴两次）
  assert.equal(
    normalizeBaseUrl("https://api.openai.com/v1/chat/completions/chat/completions").url,
    "https://api.openai.com/v1",
  );
  // 大小写不敏感
  assert.equal(
    normalizeBaseUrl("https://api.openai.com/v1/Chat/Completions").url,
    "https://api.openai.com/v1",
  );
  ...
  assert.ok(anthropic.note?.includes("messages"));
  // query/hash 剥离
  assert.equal(
    normalizeBaseUrl("https://api.example.com/v1?key=abc").url,
    "https://api.example.com/v1",
  );
  assert.equal(
    normalizeBaseUrl("https://api.example.com/v1#section").url,
    "https://api.example.com/v1",
  );
  // 普通输入不产生提示
...
test("testProviderConnection tolerates full endpoint URLs", async () => {
  ...
  try {
    ...
    assert.ok(result.normalizedNote?.includes("chat/completions"), "应提示已剥掉端点后缀");
  } finally {
    server.close();
  }
});
...
test("testProviderConnection reports HTTP error body message", async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      `error 应包含服务端消息，实际: ${result.error}`,
    );
    assert.ok(result.ms !== undefined);
  } finally {
    server.close();
