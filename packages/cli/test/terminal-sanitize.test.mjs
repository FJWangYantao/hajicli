...
test("sanitizes a complete sequence assembled from arbitrary chunks", () => {
  const chunks = ["前缀\x1b]52;", "c;QUJD\x1b", "\\中间\x1b[", "2J后缀"];
...
test("redacts legacy inline provider API keys for display and history", () => {
  ...
  assert.equal(
    redactSensitiveCommand("/provider set deepseek --project"),
    "/provider set deepseek --project",
  );
  assert.equal(
    redactSensitiveCommand("/provider set deepseek sk-secret --project"),
    "/provider set deepseek [API Key 已隐藏] --project",
  );
  assert.equal(
    redactSensitiveCommand("/provider set deepseek --project sk-secret", ""),
    "/provider set deepseek --project",
  );
  assert.equal(
    redactSensitiveCommand("/model set deepseek sk-secret-123"),
    "/model set deepseek sk-secret-123",
  );
});
