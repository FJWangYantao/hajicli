...
test("tool-pair repair removes incomplete exchanges and preserves valid ones", () => {
  ...
  const invalid = [
    { role: "system", content: "rules" },
    {
      ...
      tool_calls: [toolCall("call-a"), toolCall("call-b")],
    },
  ...
  assert.equal(
    repaired.some((message) => message.role === "tool"),
    false,
  );
  assert.equal(
    repaired.find((message) => message.content === "visible preface")?.tool_calls,
    undefined,
  );
...
test("malformed tool arguments are rejected and removed even when the tool result is paired", () => {
  ...
  assert.equal(
    repaired.some((message) => message.tool_calls?.length),
    false,
  );
  assert.equal(
    repaired.find((message) => message.content === "writing now")?.reasoning_content,
    "reasoning",
  );
...
test("L2 never keeps a head tool call after cutting away its result", () => {
  ...
  assert.equal(
    compacted.some((message) => message.tool_calls?.some((call) => call.id === "cut-at-head")),
    false,
  );
...
test("pipeline repairs an already-broken resumed history before the next provider call", async () => {
  ...
  assert.equal(
    result.messages.some((message) => message.tool_calls?.length),
    false,
  );
});
...
test("L4 sends the full history to the model and preserves the latest two user turns", async () => {
  ...
  try {
    ...
    assert.equal(
      result.messages.some((message) => message.content === "first request"),
      false,
    );
    assert.equal(
      result.messages.some((message) => message.content === "second request"),
      true,
    );
    assert.equal(
      result.messages.some((message) => message.content === "latest request"),
      true,
    );
    assert.equal(
      result.messages.find((message) => message.content === "latest request")?.snapshotId,
      "snapshot-3",
    );
...
test("automatic pipeline never snips unsummarized history below the token threshold", async () => {
  ...
  assert.equal(
    result.messages.some((message) => message.content === "important-message-20"),
    true,
  );
  assert.equal(
    result.messages.some((message) => /\[已裁切中间/.test(message.content)),
    false,
  );
});
...
test("model context window accepts a validated environment override", () => {
  ...
  assert.equal(
    getModelContextWindowTokens("deepseek-v4-flash", {
