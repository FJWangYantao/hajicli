...
import { ExperienceStore, isFailedToolOutput, jaccardSimilarity, tokenize } from "@hajicli/core";
...
test("promoteMemory moves project memory to user level as active", async () => {
  ...
  try {
    ...
    assert.ok(
      fs.existsSync(path.join(projectDir, "memory", "active", "feedback__no-auto-commit.md")),
    );
    assert.ok(
      !fs.existsSync(path.join(userDir, "memory", "active", "feedback__no-auto-commit.md")),
    );
...
test("promoteMemory promotes from staging too", async () => {
  ...
  try {
    ...
    assert.ok(
      fs.existsSync(path.join(projectDir, "memory", "staging", "project__cand-staging.md")),
    );
