...
import { ListSkillResourcesTool, LoadSkillTool, ReadSkillResourceTool } from "@hajicli/plugins";
...
test("SkillRegistry scans user and project skills with project precedence", async () => {
  ...
  try {
    ...
    assert.ok(
      result.issues.some((issue) => issue.severity === "warning" && /覆盖/.test(issue.message)),
    );
    assert.ok(
      result.issues.some(
        (issue) => issue.severity === "error" && /INVALID NAME|invalid/.test(issue.message),
      ),
    );
  } finally {
...
test("loadskill injects content once per context and supports independent child scopes", async () => {
  ...
  try {
    ...
    controller.abort();
    assert.equal(
      await tool.execute({ name: "review" }, { abortSignal: controller.signal }),
      "[Skill 加载已中止]",
    );
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
...
test("Skill resource tools require activation and safely resolve text resources", async () => {
  ...
  try {
    ...
    assert.match(
      await read.execute({ name: "review", path: "references/rules.md:stream" }),
      /不安全/,
    );
    assert.match(
      await read.execute({ name: "review", path: "references/NUL.txt" }),
      /Windows 保留设备名/,
    );
    assert.match(await read.execute({ name: "review", path: "COM1.log" }), /Windows 保留设备名/);
    assert.match(
      await read.execute({ name: "review", path: "assets/binary.dat" }),
      /不是有效 UTF-8/,
    );
...
test("Skill validation reports invalid manifests and oversized resources", async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(projectDir, "valid", "references", "large.txt"),
      "x".repeat(33),
      "utf8",
    );
    const registry = new SkillRegistry({
...
test("Skill catalog is lightweight and loadskill stays read-only in Plan Mode", async () => {
  ...
  assert.equal(
    (await engine.evaluate({ mode: "plan", toolName: "loadskill", args: {} })).action,
    "allow",
  );
...
test("L4 keeps Skill activation metadata but allows content to be reloaded", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      new RegExp(SKILL_CONTEXT_START.replace(/[[\]]/g, "\\$&")),
    );
...
test("CLI exposes deterministic Skill commands and preserves tool-call pairing", () => {
  ...
  assert.match(
    source,
    /role: "assistant",[\s\S]*?content: "",[\s\S]*?reasoning_content: "",[\s\S]*?tool_calls: \[manualSkillExchange\.toolCall\]/,
  );
  assert.match(
    source,
    /role: "assistant",[\s\S]*?content: "",[\s\S]*?reasoning_content: "",[\s\S]*?tool_calls: \[toolCall\]/,
  );
  assert.match(
    source,
    /role: "tool",\s*content: manualSkillExchange\.output,\s*tool_call_id: manualSkillExchange\.toolCall\.id/,
  );
});
