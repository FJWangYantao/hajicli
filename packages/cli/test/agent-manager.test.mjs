...
test("manual subagent command supports per-agent model, provider, effort and instructions", () => {
  assert.deepEqual(
    parseSubagentCommand(
      'bg review --model glm-5.2 --provider volcengine --effort high --instructions "只检查竞态和资源泄漏" 审查 AgentManager',
    ),
    {
      background: true,
      role: "review",
  ...
  assert.throws(
    () => parseSubagentCommand(`research --instructions "${"x".repeat(8001)}" 调研`),
    /长度必须是 1 到 8000/,
  );
  assert.equal(
    parseSubagentCommand('research --instructions "说明文字中不要使用 --model 参数" 检查配置')
      .instructions,
    "说明文字中不要使用 --model 参数",
  );
});
...
test("manual subagent command parsing guards -- separator, preset syntax and quotes", () => {
  ...
  assert.throws(
    () => parseSubagentCommand("preset:foo --preset bar 任务"),
    /不能同时使用 preset: 前缀与 --preset 选项/,
  );
  // 引号未闭合报错
  ...
  // 非十进制整数被拒绝
  assert.throws(
    () => parseSubagentCommand("research --max-tokens 1e3 任务"),
    /必须是 1000 到 2000000 之间的整数/,
  );
  assert.throws(
    () => parseSubagentCommand("research --max-tokens 0x10 任务"),
    /必须是 1000 到 2000000 之间的整数/,
  );
  ...
  // 未知选项同样视为描述开始
  assert.equal(
    parseSubagentCommand("research --unknown-flag 任务").description,
    "--unknown-flag 任务",
  );
  // 选项缺少值时报错
...
test("AgentManager tracks normalized budgets, tool calls and streaming activity", async () => {
  ...
  try {
    ...
    const launch = manager.launch(
      ...
      ({ agentId }) =>
        new Promise((resolve) => {
          finish = () => resolve(completed(agentId));
        }),
    );
...
test("AgentManager runs at most three read-only agents and pumps the queue", async () => {
  ...
  try {
    ...
    assert.equal(
      manager.list().every((agent) => agent.status === "awaiting_verification"),
      true,
    );
    assert.equal(
      manager.drainNotifications().filter((item) => item.type === "completed").length,
      4,
    );
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
...
test("queued agents can be aborted and write agents never run in parallel", async () => {
  ...
  try {
    ...
    const first = manager.launch(
      { role: "research", description: "one", background: true, access: "readonly" },
      ({ agentId }) => new Promise((resolve) => controls.set(agentId, resolve)),
    );
    const queued = manager.launch(
      { role: "review", description: "two", background: true, access: "readonly" },
      ({ agentId }) => new Promise((resolve) => controls.set(agentId, resolve)),
    );
    ...
    assert.throws(
      () =>
        manager.launch(
          ...
          async ({ agentId }) => completed(agentId),
        ),
      /不能与其他子代理并行/,
    );
...
test("verification requires parent evidence collected after child completion", async () => {
  ...
  try {
    ...
    manager.setScope("session-a");
    const launch = manager.launch(
      { role: "review", description: "review", background: false, access: "readonly" },
      async ({ agentId }) => completed(agentId, "finding"),
    );
    const finished = await launch.completion;
    assert.equal(finished.status, "awaiting_verification");
    assert.throws(
      () => manager.verify(finished.id, "verified", "looks good", []),
      /evidenceToolCallId/,
    );
    const secondLaunch = manager.launch(
      { role: "research", description: "research", background: false, access: "readonly" },
    ...
    const verified = manager.verify(finished.id, "verified", "父 Agent 重新读取了目标文件", [
      "read-parent-1",
    ]);
...
test("verification evidence and consumption survive scope reloads", async () => {
  ...
  try {
    ...
    assert.equal(
      reloaded.verify(first.id, "verified", "重新载入后使用持久化证据", ["read-parent-persisted"])
        .status,
      "verified",
    );
    ...
    assert.throws(
      () =>
        reloadedAgain.verify(second.id, "verified", "尝试复用已消费证据", [
          "read-parent-persisted",
        ]),
      /已被 Agent .* 使用/,
    );
...
test("Agent persistence retries transient rename failures and warns when locked", () => {
  ...
  try {
    ...
    assert.match(fs.readFileSync(path.join(cwd, "session-a.json"), "utf8"), /read-parent-retry/);
    assert.equal(
...
test("pending verification context is rebuilt from persisted Agent records", async () => {
  ...
  try {
    ...
    assert.match(
      context,
      new RegExp(AGENT_VERIFICATION_CONTEXT_START.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
    );
    ...
    assert.match(
      context,
      new RegExp(AGENT_VERIFICATION_CONTEXT_END.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
    );
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
...
test("verifyagent and taskfinish enforce the independent verification gate", async () => {
  ...
  try {
    ...
    store.setTaskAgent("implement", {
      id: "sub-1",
      role: "implement",
      status: "awaiting_verification",
    });
    assert.match(
      await new TaskFinishTool(store).execute({
        taskId: "implement",
    ...
    let input;
    const verify = new VerifyAgentTool(async (value) => {
      input = value;
      return "verified";
    });
    assert.match(
    ...
    assert.doesNotMatch(
      await new TaskFinishTool(store).execute({
        taskId: "implement",
        verification: "parent tests passed",
      }),
      /^错误:/,
    );
  } finally {
