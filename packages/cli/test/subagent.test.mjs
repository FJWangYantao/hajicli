...
test("subagent never executes or persists truncated tool arguments", async () => {
  ...
  assert.equal(
    calls[1].some((message) => message.role === "tool"),
    false,
  );
  assert.equal(
    calls[1].some((message) => message.tool_calls?.length),
    false,
  );
  assert.match(calls[1].at(-1).content, /系统工具调用恢复/);
  assert.equal(
    events.filter((event) => event.type === "warning" && /已拦截未执行/.test(event.message)).length,
    2,
  );
});
...
test("Plan Mode allows the subagent facade while child operations remain separately checked", async () => {
  ...
  assert.equal(
    (await engine.evaluate({ mode: "plan", toolName: "write", args: {}, userIntent: "research" }))
      .action,
    "deny",
  );
});
...
test("subagent tool validates and forwards per-agent runtime configuration", async () => {
  ...
  assert.match(
    await tool.execute({ description: "review", reasoningEffort: "extreme" }),
    /reasoningEffort/,
  );
...
test("shared executor applies hooks and records child edits for rewind", async () => {
  ...
  try {
    ...
    hooks.register("PreToolUse", (context) => {
      seen.push(["pre", context.agentId]);
    });
    hooks.register("PostToolUse", (context) => {
      seen.push(["post", context.agentId]);
    });
