...
test("parseOptionalInteger accepts empty, valid and rejects invalid input", () => {
  ...
  assert.deepEqual(parseOptionalInteger("abc", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
  assert.deepEqual(parseOptionalInteger("99", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
  assert.deepEqual(parseOptionalInteger("1001", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
  assert.deepEqual(parseOptionalInteger("1.5", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
  assert.deepEqual(parseOptionalInteger("5e2", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
  assert.deepEqual(parseOptionalInteger("0x10", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
  assert.deepEqual(parseOptionalInteger("1.0", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
  assert.deepEqual(parseOptionalInteger("+500", 100, 1000), { ok: true, value: 500 });
  assert.deepEqual(parseOptionalInteger("-5", 100, 1000), {
    ok: false,
    message: "必须是 100 到 1000 之间的整数",
  });
});
