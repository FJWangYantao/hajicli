...
test("parsePresetCommand parses name and optional fields", () => {
  assert.deepEqual(
    parsePresetCommand(
      'fast --role research --model deepseek-v4-flash --provider deepseek --effort high --instructions "只读调研" --timeout-ms 45000 --max-tokens 24000 --max-tool-calls 12',
    ),
    {
      name: "fast",
      role: "research",
  ...
  assert.deepEqual(parsePresetCommand("'调研' --effort max"), {
    name: "调研",
    reasoningEffort: "max",
  });
});
...
test("parsePresetCommand rejects invalid input", () => {
  ...
  assert.throws(
    () => parsePresetCommand("fast --effort ultra"),
    /--effort 必须是 low、medium、high、xhigh 或 max/,
  );
  assert.throws(
    () => parsePresetCommand("fast --role hacker"),
    /--role 必须是 research、review 或 implement/,
  );
...
test("updateSubagentPreset merges patch and keeps unspecified fields", () => {
  ...
  const updated = updateSubagentPreset(presets, "调研", {
    reasoningEffort: "max",
    maxTokens: 80000,
  });
