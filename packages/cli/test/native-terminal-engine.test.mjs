...
import { layoutAnsiDocumentFallback, wrapAnsiWithStateFallback } from "../dist/terminal-input.js";
...
function randomizedCases() {
  ...
  return Array.from({ length: 100 }, () => {
    const parts = Array.from({ length: 1 + (next() % 30) }, () => tokens[next() % tokens.length]);
    return {
      value: parts.join(""),
      width: 1 + (next() % 24),
