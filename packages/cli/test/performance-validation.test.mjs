...
import { findInvalidToolCall, PerformanceMonitor, validateToolCall } from "@hajicli/core";
...
test("validateToolCall accepts only complete function calls with object arguments", () => {
  ...
  assert.match(
    validateToolCall(toolCall({ function: { name: "", arguments: "{}" } })).error,
    /缺少函数名/,
  );
  assert.match(
    validateToolCall(toolCall({ function: { name: "read", arguments: null } })).error,
    /不是 JSON 字符串/,
  );
  assert.match(
    validateToolCall(toolCall({ function: { name: "read", arguments: "[]" } })).error,
    /必须是 JSON 对象/,
  );
  assert.match(
    validateToolCall(toolCall({ function: { name: "read", arguments: '{"path":' } })).error,
    /JSON 不完整或无效/,
  );
});
...
test("findInvalidToolCall returns the first invalid call with exact indices", () => {
  ...
  assert.equal(
    findInvalidToolCall([{ role: "assistant", content: "", tool_calls: [toolCall()] }]),
    null,
  );
});
