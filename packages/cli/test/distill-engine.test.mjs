...
test("runDistill detects edit-before-read pattern when Read is missing", async () => {
  ...
  try {
    ...
    assert.ok(
      instincts.some((i) => i.id === "edit-before-read"),
      "应检测到 edit-before-read",
    );
  } finally {
  ...
  try {
    ...
    const observations = [
      obs({
        ...
        output: "执行出错: ENOENT module not found",
        failed: true,
      }),
      obs({
        toolName: "bash",
        args: { command: "pnpm test" },
        output: "执行出错: ENOENT another",
        failed: true,
      }),
      obs({
        toolName: "bash",
        args: { command: "pnpm test" },
        output: "执行出错: ENOENT third",
        failed: true,
      }),
    ];
    const summary = await engine.runDistill(observations, {
    ...
    const instincts = store.loadInstincts(true);
    assert.ok(
      instincts.some((i) => i.domain === "error-prevention"),
      "应检测到 error-prevention 规则",
    );
  } finally {
    fsp.rm(tmp, { recursive: true, force: true });
  }
  ...
  try {
    ...
    const observations = [
      obs({
        ...
        output: "执行出错: not found",
        failed: true,
      }),
      obs({
        toolName: "bash",
        args: { command: "node y.js" },
        output: "执行出错: not found",
        failed: true,
      }),
    ];
  ...
  try {
    ...
    const llmResponse = JSON.stringify({
      instincts: [
        {
          id: "read-before-edit",
          trigger: "when edit file",
          action: "read file before edit workflow",
          domain: "workflow",
        },
        {
          id: "read-file-before-edit",
          trigger: "before edit file",
          action: "read file first edit workflow",
          domain: "workflow",
        },
      ],
      memories: [],
