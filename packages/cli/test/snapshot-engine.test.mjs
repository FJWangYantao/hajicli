...
test("snapshot creation reports storage failures instead of silently disabling rewind", () => {
  ...
  try {
    const engine = new SnapshotEngine(cwd, path.join(blocker, "snapshots"), (warning) =>
...
test("project Skill files remain protected by snapshot rewind", () => {
  ...
  try {
    fs.writeFileSync(
      ...
      ["**/.haji/", "!/.haji/", "/.haji/*", "!/.haji/skills/", "!/.haji/skills/**", ""].join("\n"),
    );
...
test("rewind only reverts Haji-owned tool changes and preserves later external edits", () => {
  ...
  try {
    ...
    assert.equal(
      fs.readFileSync(path.join(cwd, "tracked.txt"), "utf8"),
      "changed later by external editor\n",
    );
    assert.equal(
      fs.readFileSync(path.join(cwd, "external.txt"), "utf8"),
...
test("lightweight anchors and scoped edit checkpoints only track the target file", () => {
  ...
  try {
    ...
    assert.equal(
      fs.readFileSync(path.join(cwd, "external.txt"), "utf8"),
      "changed externally during tool\n",
    );
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
