...
export function formatPendingAgentVerificationContext(records: readonly AgentRecord[]): string {
  const pending = records.filter(
    (record) => record.status === "awaiting_verification" && record.result,
  );
...
export function normalizeSubagentTimeoutMs(
  ...
  return Math.max(
    ...
    Math.min(MAX_SUBAGENT_TIMEOUT_MS, Math.trunc(candidate)),
  );
}
...
function isAgentRecord(value: unknown): value is AgentRecord {
  ...
  return (
    ...
    (record.reasoningEffort === undefined ||
      ["low", "medium", "high", "xhigh", "max"].includes(String(record.reasoningEffort))) &&
...
export class AgentManager {
  ...
  setScope(scope: string): void {
    ...
    for (const [id, runtime] of this.runtimes) {
      ...
      if (runtime.parentSignal && runtime.parentAbort)
        runtime.parentSignal.removeEventListener("abort", runtime.parentAbort);
    ...
    for (const record of state.records) {
      ...
      record.maxToolCalls = normalizeSubagentMaxToolCalls(
        record.maxToolCalls,
        DEFAULT_SUBAGENT_MAX_TOOL_CALLS,
      );
      record.toolCalls = Number.isFinite(record.toolCalls)
        ? Math.max(0, Math.trunc(record.toolCalls))
        : 0;
  ...
  launch(request: AgentLaunchRequest, executor: AgentExecutor): AgentLaunch {
    ...
    if (
      ...
      [...this.records.values()].some((item) => ["queued", "running"].includes(item.status))
    ) {
  ...
  updateProgress(agentId: string, activity: "thinking" | "responding", delta: string): void {
    const record = this.records.get(agentId);
    if (record?.status !== "running" || !delta) return;
  ...
  recordParentEvidence(toolCallId: string, toolName: string, timestamp = Date.now()): void {
    if (
      !toolCallId ||
      ["subagent", "verifyagent"].includes(toolName) ||
    ...
    if (this.parentEvidence.length > 200)
      this.parentEvidence.splice(0, this.parentEvidence.length - 200);
    try {
      this.persist();
    } catch (error) {
      this.options.onWarning?.(
        `验证证据持久化失败：${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  verify(
    ...
    if (record.status !== "awaiting_verification")
      throw new Error(`Agent ${agentId} 当前不等待验证`);
    ...
    const evidenceItems = requestedIds.map((toolCallId) => {
      ...
      if (item.timestamp < (record.finishedAt || 0))
        throw new Error(`验证证据早于子代理完成时间: ${toolCallId}`);
      if (item.consumedByAgentId)
        throw new Error(`验证证据 ${toolCallId} 已被 Agent ${item.consumedByAgentId} 使用`);
      return item;
    });
    ...
    this.notify(
      record,
      verdict,
      verdict === "verified" ? "父 Agent 已独立验证结果" : "父 Agent 已拒绝结果",
    );
    this.persistAndEmit();
  ...
  private shouldQueue(record: AgentRecord): boolean {
    ...
    const writeRunning = [...this.records.values()].some(
      (item) => item.status === "running" && item.access === "workspace-write",
    );
  ...
  private runningReadonlyCount(): number {
    return [...this.records.values()].filter(
      (record) => record.status === "running" && record.access === "readonly",
    ).length;
  }
  ...
  private readState(): ReadAgentStateResult {
    try {
      ...
      if (
        ...
        !Array.isArray(state.parentEvidence)
      ) {
        throw new Error("状态文件结构或版本无效");
      }
      if (!state.records.every(isAgentRecord)) throw new Error("Agent 记录包含无效项目");
      if (!state.parentEvidence.every(isParentEvidence))
        throw new Error("验证证据账本包含无效项目");
      return {
        status: "loaded",
        records: state.records,
        parentEvidence: state.parentEvidence,
        legacy: false,
      };
    } catch (error) {
