import { type ExperienceStore, jaccardSimilarity, tokenize } from "./experience-store.js";
import type {
  DistillSummary,
  Instinct,
  InstinctDomain,
...
export class DistillEngine {
  ...
  async runDistill(
    ...
    const shouldTriggerLLM =
      failedCount >= LLM_TRIGGER_MIN_FAILURES || sessionObservations.length >= LLM_TRIGGER_MIN_OBS;
    ...
    if (shouldTriggerLLM) {
      ...
      try {
        ...
        notes.push(
          `LLM 路径产出 ${llmInstincts.length} 条规则、${memoryCandidates.length} 条记忆候选`,
        );
      } catch (error) {
      ...
      notes.push(
        `LLM 路径未触发（失败 ${failedCount} 次，观测 ${sessionObservations.length} 条，阈值 ${LLM_TRIGGER_MIN_OBS}）`,
      );
    }
  ...
  private detectBashFailureRetry(obs: ToolObservation[]): Instinct[] {
    ...
    for (const fail of failedCommands) {
      ...
      const retrySucceeded = bash.some(
        (o) =>
          !o.failed &&
  ...
  private detectGrepThenRead(obs: ToolObservation[]): Instinct[] {
    ...
    for (const grep of greps) {
      ...
      const hasFollowUpRead = obs.some(
        (o) =>
          o.toolName === TOOL.READ &&
  ...
  private detectSubagentVerify(obs: ToolObservation[]): Instinct[] {
    ...
    for (const sub of subagentCalls) {
      ...
      const hasVerify = obs.some(
        (o) => o.toolName === TOOL.VERIFY && new Date(o.ts).getTime() > subTime,
      );
  ...
  private detectRepeatedWriteReplace(obs: ToolObservation[]): Instinct[] {
    ...
    const repeated = Array.from(byFile.entries()).filter(
      ([, count]) => count >= STATISTICAL_MIN_OCCURRENCES,
    );
  ...
  private detectFailedPatternCluster(obs: ToolObservation[]): Instinct[] {
    ...
    for (const [signature, samples] of clusters) {
      ...
      const sampleCmd =
        samples[0].args?.command || samples[0].args?.file_path || samples[0].args?.path || "";
  ...
  private deduplicateInstincts(instincts: Instinct[]): Instinct[] {
    ...
    return Array.from(groups.values()).map((group) =>
      group.reduce((best, cur) =>
        cur.trigger.length + cur.action.length > best.trigger.length + best.action.length
          ? cur
          : best,
      ),
    );
...
function extractErrorSignature(output: string): string {
  ...
  const keywords = output.match(
    /(ENOENT|EACCES|ECONNREFUSED|EPERM|not found|undefined|null|timeout|timed out|refused|unauthorized|invalid|failed)/i,
  );
  if (keywords) return keywords[0].toLowerCase();
  // 退回到错误前缀的前若干字符（去掉标点，保留信息量）
  if (output.startsWith("执行出错:"))
    return output
      .slice(0, 40)
