import type { SkillActivation, SkillSource } from "./skill-types.js";
...
function parseActivation(value: string): SkillActivation | null {
  try {
    ...
    if (!parsed.name || !parsed.contentHash || !["user", "project"].includes(String(parsed.source)))
      return null;
    return {
...
export function extractSkillActivations(
  messages: ChatMessage[],
): Array<SkillActivation & { resident: boolean }> {
