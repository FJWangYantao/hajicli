...
export class TaskStore {
  ...
  getPlan(): TaskPlan | null {
    try {
      ...
      const raw = JSON.parse(fs.readFileSync(filePath, "utf8")) as Partial<TaskPlan> & {
        summary?: string;
      };
      const migrated: TaskPlan = {
        title: String(raw.title || raw.summary || ""),
        tasks: Array.isArray(raw.tasks)
          ? raw.tasks.map((task) => this.normalizeTask(task as TaskItem))
          : [],
  ...
  createTask(input: {
    ...
    if ([...tasks, ...completedTasks].some((task) => task.id === id))
      throw new Error(`任务 ID 重复: ${id}`);

    const blockedBy = [
      ...new Set((input.blockedBy || []).map((value) => value.trim()).filter(Boolean)),
    ];
  ...
  updateTask(
    taskId: string,
    updates: { status?: TaskStatus; content?: string; blockedBy?: string[] },
  ): TaskPlan {
  ...
  finishTask(
    taskId: string,
    verification: string,
  ): { plan: TaskPlan; finished: TaskItem; allDone: boolean } {
    ...
    if (task.status !== "in_progress") {
      throw new Error(
        `任务必须先更新为 in_progress。请先调用 updatetask({ taskId: "${task.id}", status: "in_progress" })`,
      );
    }
  ...
  clearTasks(): void {
    try {
      fs.rmSync(this.getFilePath(), { force: true });
    } catch {}
  }
