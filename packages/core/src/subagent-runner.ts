...
export function normalizeSubagentMaxTokens(
  ...
  return Math.max(
    ...
    Math.min(MAX_SUBAGENT_MAX_TOKENS, Math.trunc(candidate)),
  );
}
...
export function normalizeSubagentMaxToolCalls(
  ...
  return Math.max(
    ...
    Math.min(MAX_SUBAGENT_MAX_TOOL_CALLS, Math.trunc(candidate)),
  );
}
  ...
  async runResult(
    ...
    if ((parentContext.depth || 0) > 0) {
      return {
        ...
        status: "failed",
        summary:
          "当前版本禁止子代理递归委派。请使用现有只读工具串行完成当前子任务，并把无法完成的部分写入 unresolved 交由父 Agent 处理。",
        filesChanged: [],
        verification: [],
    ...
    const messages: ChatMessage[] = [
      ...
      {
        role: "user",
        content: request.taskId ? `任务 ${request.taskId}：${description}` : description,
      },
    ];
    ...
    try {
      for (let turn = 0; turn < (this.options.maxTurns || 20); turn += 1) {
        ...
        const reportedTotalTokens =
          turnUsage && Number.isFinite(turnUsage.total_tokens)
      ...
      const timedOut =
        aborted && abortReason instanceof Error && abortReason.name === "TimeoutError";
      const result: SubagentResult = {
        agentId,
