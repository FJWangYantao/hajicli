...
export class SkillRegistry {
  ...
  private async scanRoot(
    ...
    try {
      ...
      report(
        "error",
        `${source} Skill 目录解析失败${fsErrorCode(error) ? ` (${fsErrorCode(error)})` : ""}`,
      );
      return;
    }
    for (const directory of directories.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      try {
        ...
        if (stat.size > this.maxSkillBytes)
          throw new Error(`SKILL.md 超过 ${this.maxSkillBytes} 字节限制`);
        const content = await fs.readFile(realManifest, "utf8");
        const { metadata, body } = parseFrontmatter(content);
        const name = singleLine(metadata.name) || directory.name;
        if (!SKILL_NAME_PATTERN.test(name))
          throw new Error("name 仅允许小写字母、数字、-、_，长度 1-64");
        ...
        if (previous)
          report("warning", `Skill "${name}"：${source} 来源覆盖 ${previous.source} 来源。`, name);
        ...
        if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
          ...
          const detail = code
            ...
            : error instanceof Error
              ? error.message
              : String(error);
          report("error", `已忽略 ${source} Skill "${directory.name}"：${detail}`, directory.name);
        }
      }
    }
  }
  ...
  load(name: string, args?: string, scope = "main"): string {
    ...
    if (previous?.resident && previous.contentHash === skill.contentHash) {
      return [
        ...
        invocationArgs
          ? `本次调用参数：${invocationArgs}`
          : "该 Skill 当前已在上下文中，无需重复注入。",
      ].join("\n");
    }
    ...
    for (const skill of this.listEntries()) {
      ...
      try {
        ...
        for (const resource of result.resources) {
          ...
          try {
            absolutePath = (await this.assertResourcePath(skill, resource.path, signal))
              .absolutePath;
          } catch (error) {
            issues.push({
              severity: "error",
