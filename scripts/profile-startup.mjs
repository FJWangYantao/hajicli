...
for (let i = 0; i < rounds; i++) {
  ...
  const exitCode = await new Promise((resolve) => {
    const timer = setTimeout(() => {
      console.error(
        `round ${i + 1}: 子进程超时（15s），强制终止。stderr 前 2000 字符:\n${stderr.slice(0, 2000)}`,
      );
      child.kill();
    }, 15000);
    child.on("exit", (code) => {
  ...
  results.push(record);
  console.log(
    `round ${record.round}: total=${record.totalMs}ms spawn→ready=${record.spawnToReady ?? "N/A"}ms exit=${record.exitCode} ready=${record.marks.ready ?? "N/A"}ms`,
  );
}
...
console.log(
  "阶段".padEnd(16),
  [...Array(rounds)].map((_, i) => `R${i + 1}`.padStart(10)).join(""),
  "中位数".padStart(10),
);
for (const stage of STAGE_ORDER) {
  const increments = results
    .map((r) => {
